            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-body">
                    <form method="GET" action="{{ route('transfers.index') }}" class="row g-2 align-items-center">
                        <div class="col-md-2">
                            <input type="text" name="search" class="form-control" placeholder="Transfer #"
                                ...
                                <select name="from_branch_id" class="form-select">
                                    ...
                                    @foreach ($branches as $branch)
                                        <option value="{{ $branch->id }}"
                                            {{ (string) request('from_branch_id') === (string) $branch->id ? 'selected' : '' }}>
                                            {{ $branch->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                                ...
                                <select name="from_store_id" class="form-select">
                                    ...
                                    @foreach ($stores as $store)
                                        <option value="{{ $store->id }}"
                                            {{ (string) request('from_store_id') === (string) $store->id ? 'selected' : '' }}>
                                            {{ $store->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        @isset($branches)
                            <div class="col-md-2">
                                <select name="to_branch_id" class="form-select">
                                    ...
                                    @foreach ($branches as $branch)
                                        <option value="{{ $branch->id }}"
                                            {{ (string) request('to_branch_id') === (string) $branch->id ? 'selected' : '' }}>
                                            {{ $branch->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        @isset($stores)
                            <div class="col-md-2">
                                <select name="to_store_id" class="form-select">
                                    ...
                                    @foreach ($stores as $store)
                                        <option value="{{ $store->id }}"
                                            {{ (string) request('to_store_id') === (string) $store->id ? 'selected' : '' }}>
                                            {{ $store->name }}</option>
                                    @endforeach
                                </select>
                            </div>
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    <div class="table-responsive">
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-end gap-1 flex-wrap">
                                                ...
                                                @if ($transfer->status === 'pending')
                                                    ...
                                                    <a href="{{ route('transfers.approve', $transfer) }}"
                                                ...
                                                @if (in_array($transfer->status, ['approved', 'in_transit']))
                                                    <a href="{{ route('transfers.receive', $transfer) }}"
                                                        class="btn btn-sm btn-outline-info" title="Receive">
                                                ...
                                                @if (in_array($transfer->status, ['pending', 'approved']))
                                                    ...
                                                    <div class="modal fade" id="cancelTransferModal{{ $transfer->id }}"
                                                            ...
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-footer">
                                                                    ...
                                                                    <form method="POST"
                                                                        ...
                                                                        @method('DELETE')
                                                                        <button type="submit"
                                                                            class="btn btn-danger">Cancel Transfer</button>
                                                                    </form>
                                                                </div>
                                                            </div>
                                                        </div>
                        ...
                        @if (
                            $transfers instanceof \Illuminate\Contracts\Pagination\Paginator ||
