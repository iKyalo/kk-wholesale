                ...
                <div class="col-lg-8">
                    <div class="card border-0 shadow-sm">
                        <div class="card-body p-4">
                            <form method="POST" action="{{ route('inventory.update-stock') }}"
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select class="form-select @error('branch_id') is-invalid @enderror" id="branch_id"
                                            ...
                                            <option value="" disabled
                                                {{ old('branch_id', $inventory->store->branch_id ?? '') ? '' : 'selected' }}>
                                                Select a branch
                                            </option>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <select class="form-select @error('product_id') is-invalid @enderror"
                                            ...
                                            <option value="" disabled
                                                {{ old('product_id', $inventory->product_id ?? '') ? '' : 'selected' }}>
                                                Select a product
                                            </option>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select class="form-select @error('adjustment_type') is-invalid @enderror"
                                            ...
                                            <option value="stock_in"
                                                {{ old('adjustment_type') === 'stock_in' ? 'selected' : '' }}>Stock In
                                            </option>
                                            <option value="stock_out"
                                                {{ old('adjustment_type') === 'stock_out' ? 'selected' : '' }}>Stock Out
                                            </option>
                                            <option value="set_exact"
                                                {{ old('adjustment_type') === 'set_exact' ? 'selected' : '' }}>Set Exact
                                                Quantity</option>
                                        </select>
                                    ...
                                    <div class="col-md-4">
                                        <label for="reference" class="form-label">Reference Number <span
                                                class="text-muted fw-normal text-sm">(optional)</span></label>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="3"
                                            placeholder="Reason for this adjustment">{{ old('notes') }}</textarea>
                                        @error('notes')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
            ...
            function filterStores() {
                ...
                Array.from(storeSelect.options).forEach(option => {
                    ...
                    if (storeBranchId === branchId) {
                        ...
                        // Restore selected store if it belongs to the selected branch
                ...
                storeSelect.disabled = !branchId;

                // If no branch is selected, show the placeholder
                if (!branchId) {
                    storeSelect.value = '';
                }
            }

            // Filter stores when branch changes
