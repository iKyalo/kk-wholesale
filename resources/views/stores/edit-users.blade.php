...
@section('content')
    @php
        ...
        // selections survive, otherwise fall back to the store's current users.
$assignedIds = old(
    'users',
    optional($store->users ?? collect())
        ->pluck('id')
        ->toArray() ?? [],
);
...
$roleBadgeMap = [
    ...
    <div class="container-fluid py-4">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-start mb-4 gap-2">
            <div>
                <h1 class="h3 mb-1">Edit Store Users</h1>
                <p class="text-muted mb-0">Manage users assigned to {{ $store->name }}</p>
            </div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="row row-cols-1 row-cols-sm-2 row-cols-lg-5 g-3">
                    ...
                    <div class="col">
                        <div class="text-muted small">Parent Branch</div>
                        <div>
                            @if (!empty($store->branch))
                                <span class="badge bg-primary-subtle text-primary-emphasis">
                                    <i class="bi bi-diagram-3 me-1"></i>{{ $store->branch->name }}
                                </span>
                            @else
        ...
        <form action="{{ route('stores.users.update', $store) }}" method="POST" id="storeUsersForm">
            @csrf
            @method('PUT')
                                            ...
                                            <td>
                                                <div class="form-check mb-0">
                                                    <input class="form-check-input user-checkbox" type="checkbox"
                                                        ...
                                                        aria-label="Assign {{ $user->name }} to this store">
                                                </div>
                                            </td>
                                            <td>
...
@push('scripts')
    <script>
        $(function() {
            ...
            $('#storeUsersForm').on('submit', function() {
