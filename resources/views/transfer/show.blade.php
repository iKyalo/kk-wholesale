    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between mb-4 gap-2">
        <div>
            ...
            <p class="text-muted small mb-0">
                {{ $transfer->transfer_date->format('d M Y') }} &middot; Requested by {{ $transfer->requestedBy->name ?? '—' }}
            </p>
        </div>
                    ...
                    <tbody>
                        @forelse ($transfer->items as $item)
                            <tr>
                                ...
                                <td class="text-end">{{ $item->quantity_dispatched !== null ? number_format($item->quantity_dispatched) : '—' }}</td>
                                <td class="text-end">{{ $item->quantity_received !== null ? number_format($item->quantity_received) : '—' }}</td>
                            </tr>
                        @empty
                            <tr>
                ...
                <div class="card-body">
                    @forelse ($transfer->events ?? [] as $event)
                        <div class="d-flex mb-3">
                            <div class="me-3">
                                <span class="bg-primary bg-opacity-10 text-primary rounded-circle d-flex align-items-center justify-content-center" style="width: 32px; height: 32px;">
                            ...
                            <div>
                                ...
                                <p class="text-muted small mb-0">
                                    {{ $event->user->name ?? '—' }} &middot; {{ $event->created_at->format('d M Y, H:i') }}
                                </p>
                                @if ($event->notes)
