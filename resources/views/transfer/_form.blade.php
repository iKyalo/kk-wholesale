{{--
    Expects: $branches, $stores, $products
    Optional: $transfer (when editing) — used to prepopulate fields and items.
    Each $store is expected to expose branch_id.
    Each $product is expected to expose stock_by_store as [store_id => quantity].
--}}
...
<div class="row g-4">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-3">
                ...
                <select class="form-select @error('source_branch_id') is-invalid @enderror" id="source_branch_id" name="source_branch_id" required>
                    <option value="" disabled {{ old('source_branch_id', $transfer->sourceStore->branch_id ?? '') ? '' : 'selected' }}>Select branch</option>
                    @foreach ($branches as $branch)
                        <option value="{{ $branch->id }}" {{ (string) old('source_branch_id', $transfer->sourceStore->branch_id ?? '') === (string) $branch->id ? 'selected' : '' }}>
                            {{ $branch->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-3">
                ...
                <select class="form-select @error('source_store_id') is-invalid @enderror" id="source_store_id" name="source_store_id" required>
                    <option value="" disabled {{ old('source_store_id', $transfer->source_store_id ?? '') ? '' : 'selected' }}>Select store</option>
            ...
            <div class="col-md-3">
                ...
                <select class="form-select @error('destination_branch_id') is-invalid @enderror" id="destination_branch_id" name="destination_branch_id" required>
                    <option value="" disabled {{ old('destination_branch_id', $transfer->destinationStore->branch_id ?? '') ? '' : 'selected' }}>Select branch</option>
                    @foreach ($branches as $branch)
                        <option value="{{ $branch->id }}" {{ (string) old('destination_branch_id', $transfer->destinationStore->branch_id ?? '') === (string) $branch->id ? 'selected' : '' }}>
                            {{ $branch->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-3">
                ...
                <select class="form-select @error('destination_store_id') is-invalid @enderror" id="destination_store_id" name="destination_store_id" required>
                    <option value="" disabled {{ old('destination_store_id', $transfer->destination_store_id ?? '') ? '' : 'selected' }}>Select store</option>
                    @foreach ($stores as $store)
                        <option
                            value="{{ $store->id }}"
                            data-branch-id="{{ $store->branch_id }}"
                ...
                @error('destination_store_id')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                <div id="sameStoreWarning" class="form-text text-danger d-none">Source and destination stores must be different.</div>
            </div>
            ...
            <div class="col-md-8">
                ...
                <textarea
                    ...
                    name="notes"
                    rows="1"
                    placeholder="Reason for transfer or handling instructions"
    ...
    <div class="col-12">
        ...
        <input type="text" id="productSearchInput" class="form-control mb-3" placeholder="Search products by name or SKU...">
                ...
                <tbody>
                    @forelse ($products as $product)
                        ...
                        >
                            ...
                            <td>
                                <input type="number" class="form-control form-control-sm product-qty-input" min="1" value="1">
                            </td>
                            <td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateSummary() {
        ...
        document.getElementById('summaryTotalUnits').textContent = items.reduce(function (sum, i) { return sum + i.quantity; }, 0);

        const sourceOption = sourceStoreSelect.selectedOptions[0];
        const destOption = destinationStoreSelect.selectedOptions[0];
        document.getElementById('summarySourceStore').textContent = sourceOption && sourceOption.value ? sourceOption.textContent : '—';
        document.getElementById('summaryDestinationStore').textContent = destOption && destOption.value ? destOption.textContent : '—';
    }
